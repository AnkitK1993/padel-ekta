...
function renderHome() {
  ...
  const cardHtmls = stats.map((p, i) => {
    const rc = i === 0 ? "r1" : i === 1 ? "r2" : i === 2 ? "r3" : i === 3 ? "r4" : i === 4 ? "r5" : "";
    const ri =
      i === 0
        ...
        : i === 1
          ...
          : i === 2
            ...
            : i === 3
            ? `<svg viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" style="display:block"><defs><radialGradient id="mg4" cx="36%" cy="28%" r="72%"><stop offset="0%" stop-color="#c8ccff"/><stop offset="28%" stop-color="#7080ee"/><stop offset="65%" stop-color="#4450bb"/><stop offset="100%" stop-color="#283080"/></radialGradient><radialGradient id="mg4HL" cx="30%" cy="22%" r="44%"><stop offset="0%" stop-color="rgba(255,255,255,0.55)"/><stop offset="100%" stop-color="rgba(255,255,255,0)"/></radialGradient></defs><circle cx="20" cy="20" r="19.5" fill="#10104a"/><circle cx="20" cy="20" r="18" fill="url(#mg4)"/><circle cx="20" cy="20" r="18" fill="url(#mg4HL)"/><circle cx="20" cy="20" r="13.5" fill="none" stroke="rgba(180,190,255,0.5)" stroke-width="1.2"/><text x="20" y="27.5" text-anchor="middle" font-family="'Bebas Neue',sans-serif" font-size="19" font-weight="900" fill="#10104a">4</text></svg>`
            : i === 4
            ? `<svg viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" style="display:block"><defs><radialGradient id="mg5" cx="36%" cy="28%" r="72%"><stop offset="0%" stop-color="#e0baff"/><stop offset="28%" stop-color="#aa60ee"/><stop offset="65%" stop-color="#7030bb"/><stop offset="100%" stop-color="#401870"/></radialGradient><radialGradient id="mg5HL" cx="30%" cy="22%" r="44%"><stop offset="0%" stop-color="rgba(255,255,255,0.55)"/><stop offset="100%" stop-color="rgba(255,255,255,0)"/></radialGradient></defs><circle cx="20" cy="20" r="19.5" fill="#20082e"/><circle cx="20" cy="20" r="18" fill="url(#mg5)"/><circle cx="20" cy="20" r="18" fill="url(#mg5HL)"/><circle cx="20" cy="20" r="13.5" fill="none" stroke="rgba(220,160,255,0.5)" stroke-width="1.2"/><text x="20" y="27.5" text-anchor="middle" font-family="'Bebas Neue',sans-serif" font-size="19" font-weight="900" fill="#20082e">5</text></svg>`
            : i + 1;
  ...
  if (splashDone) {
    ...
    if (matchRowHtmls.length) {
      ...
      const animRows = reversedMatches
        .slice(0, animCount)
        .map((m) =>
          buildMatchRowHtml(m, " row-reveal-anim", null, allMatches.indexOf(m)),
        );
...
function endLiveMatch() {
  ...
  renderModernMatches();
  showToast(
    `Saved! ${a1} & ${a2} ${_liveScoreA}–${_liveScoreB} ${b1} & ${b2}`,
    "🎾",
  );
  goTo("home");
}
...
function openMatchIntro(idx) {
  ...
  document.getElementById("mio-date-bar").textContent = fmtDate(
    m.date,
  ).toUpperCase();
