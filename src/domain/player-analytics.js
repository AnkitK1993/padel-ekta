...
// Generates narrative story events from a match sequence.
// Returns [] if fewer than 2 matches. Each story: { icon, type, text, date }.
export function computeMatchStories(matches) {
  ...
  sorted.forEach((m, idx) => {
    ...
    m.teamA.forEach((p) => {
      ...
      assHistory[p] = (assHistory[p] || 1000) + (aWon ? Math.round(quality * mult) : -Math.round(quality / mult));
    });
    m.teamB.forEach((p) => {
      ...
      [1050, 1100, 1150, 1200, 1250].forEach((milestone) => {
        if (prevAss[p] < milestone && assHistory[p] >= milestone)
          stories.push({ icon: "⭐", type: "ass-milestone",
            ...
            date, score: `ASS ${Math.round(assHistory[p])}`, matchIdx: idx });
      });
    });
    ...
    if (m.scoreA === 0 || m.scoreB === 0) {
      ...
      stories.push({ icon: "💀", type: "shutout",
        text: `${winner.join(" & ")} shut out ${loser.join(" & ")} ${m.scoreA}–${m.scoreB}`,
        date, score: `${m.scoreA}–${m.scoreB}`, matchIdx: idx });
    }
  });

  // All-time win streak records
  computeStats(matches).forEach((p) => {
    if (p.bestWinStreak >= 7)
      stories.push({ icon: "🔥", type: "streak",
        text: `${p.name} holds the all-time record: ${p.bestWinStreak}-match win streak`,
        date: null, score: `${p.bestWinStreak} wins`, matchIdx: null });
  });
...
// Given the matches where playerA and playerB were partners, breaks down
// which opponents they faced and how often — for the Partner/Opponent Grid's
// drill-down ("when A & B partnered, who did they play against, how often").
export function computeOpponentBreakdown(matches, playerA, playerB, norm = (n) => n) {
  ...
  (matches || []).forEach((m) => {
    ...
    const B = (m.teamB || []).map(norm);
    let opp;
    if (A.includes(a) && A.includes(b)) opp = B;
    else if (B.includes(a) && B.includes(b)) opp = A;
    else return;
    total++;
    opp.forEach((o) => {
